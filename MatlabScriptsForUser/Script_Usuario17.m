...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Rwave, locs_Rwave] = findpeaks(temp, MinPeakHeight=70, MinPeakDistance=40);
    ...
    [Amp_Swave, locs_Swave] = findpeaks(-temp, MinPeakHeight=40, MinPeakDistance=40);

    %Detección Puntos Característico Q
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=30, MinPeakDistance=7);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-32);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-32);
...
writetable(T1, 'RiseTime15.txt');
writetable(T2, 'FallTime15.txt');
writetable(T3, 'RiseLevel15.txt');
writetable(T4, 'FallLevel15.txt');