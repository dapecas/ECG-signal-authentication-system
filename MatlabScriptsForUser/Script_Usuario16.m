...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Swave, locs_Swave] = findpeaks(-temp, MinPeakHeight=130, MinPeakDistance=40);

    %Detección Puntos Característico Q
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=50, MinPeakDistance=7);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<0);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<0);
