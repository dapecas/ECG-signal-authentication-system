...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Swave, locs_Swave] = findpeaks(-temp, MinPeakHeight=90, MinPeakDistance=40);

    %Detección Puntos Característico Q
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=50, MinPeakDistance=0.9);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-100);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-100);
...
writetable(T1, 'RiseTime6.txt');
writetable(T2, 'FallTime6.txt');
writetable(T3, 'RiseLevel6.txt');
writetable(T4, 'FallLevel6.txt');