...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=50, MinPeakDistance=0.1);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-65);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-65);
...
writetable(T1, 'RiseTime12.txt');
writetable(T2, 'FallTime12.txt');
writetable(T3, 'RiseLevel12.txt');
writetable(T4, 'FallLevel12.txt');