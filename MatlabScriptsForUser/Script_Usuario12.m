...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Qwave, locs_Qwave] = findpeaks(-temp, MinPeakHeight=50, MinPeakDistance=40);

    %Detección Puntos Característico S
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=25, MinPeakDistance=7);
    locs_Swave = min_locs(temp(min_locs)>-min(Amp_Qwave) & temp(min_locs)<-44);
    Amp_Swave = min_amp(temp(min_locs)>-min(Amp_Qwave) & temp(min_locs)<-44);
...
writetable(T1, 'RiseTime11.txt');
writetable(T2, 'FallTime11.txt');
writetable(T3, 'RiseLevel11.txt');
writetable(T4, 'FallLevel11.txt');