...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Rwave, locs_Rwave] = findpeaks(temp, MinPeakHeight=40, MinPeakDistance=40);
    ...
    [Amp_Swave, locs_Swave] = findpeaks(-temp, MinPeakHeight=30, MinPeakDistance=40);

    %Detección Puntos Característico Q
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=19, MinPeakDistance=7);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-24);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-24);
...
writetable(T1, 'RiseTime13.txt');
writetable(T2, 'FallTime13.txt');
writetable(T3, 'RiseLevel13.txt');
writetable(T4, 'FallLevel13.txt');