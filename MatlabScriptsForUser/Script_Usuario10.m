...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=20, MinPeakDistance=7.2);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-30);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-30);
...
writetable(T1, 'RiseTime9.txt');
writetable(T2, 'FallTime9.txt');
writetable(T3, 'RiseLevel9.txt');
writetable(T4, 'FallLevel9.txt');