...
hold on;
plot(noisyECG_withTrend, MarkerFaceColor= 'r'); %Comparación con la señal origin
...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Qwave, locs_Qwave] = findpeaks(-temp, MinPeakHeight=100, MinPeakDistance=40);

    %Detección Puntos Característico S
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=90, MinPeakDistance=7);
    locs_Swave = min_locs(temp(min_locs)>-min(Amp_Qwave) & temp(min_locs)<-90);
    Amp_Swave = min_amp(temp(min_locs)>-min(Amp_Qwave) & temp(min_locs)<-90);
...
writetable(T1, 'RiseTime2.txt');
writetable(T2, 'FallTime2.txt');
writetable(T3, 'RiseLevel2.txt');
writetable(T4, 'FallLevel2.txt');