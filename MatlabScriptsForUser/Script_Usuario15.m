...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Swave, locs_Swave] = findpeaks(-temp, MinPeakHeight=65, MinPeakDistance=40);

    %Detección Puntos Característico Q
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=40, MinPeakDistance=7);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-42);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-42);
...
writetable(T1, 'RiseTime14.txt');
writetable(T2, 'FallTime14.txt');
writetable(T3, 'RiseLevel14.txt');
writetable(T4, 'FallLevel14.txt');