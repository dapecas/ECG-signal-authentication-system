...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=15, MinPeakDistance=9.9);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-23);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-23);
...
writetable(T1, 'RiseTime8.txt');
writetable(T2, 'FallTime8.txt');
writetable(T3, 'RiseLevel8.txt');
writetable(T4, 'FallLevel8.txt');