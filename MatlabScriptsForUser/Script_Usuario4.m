...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [Amp_Swave, locs_Swave] = findpeaks(-temp, MinPeakHeight=45, MinPeakDistance=40);

    %Detección Puntos Característico Q
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=45, MinPeakDistance=0.9);
    locs_Qwave = min_locs(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-40);
    Amp_Qwave = min_amp(temp(min_locs)>-min(Amp_Swave) & temp(min_locs)<-40);
...
writetable(T1, 'RiseTime3.txt');
writetable(T2, 'FallTime3.txt');
writetable(T3, 'RiseLevel3.txt');
writetable(T4, 'FallLevel3.txt');