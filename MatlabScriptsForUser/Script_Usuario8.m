...
temp=ECG_filtered((i-1)*w+1:(i)*w);
    ...
    [min_amp,min_locs] = findpeaks(-temp, MinPeakHeight=50, MinPeakDistance=7);
    locs_Swave = min_locs(temp(min_locs)>-min(Amp_Qwave) & temp(min_locs)<-65);
    Amp_Swave = min_amp(temp(min_locs)>-min(Amp_Qwave) & temp(min_locs)<-65);
...
writetable(T1, 'RiseTime7.txt');
writetable(T2, 'FallTime7.txt');
writetable(T3, 'RiseLevel7.txt');
writetable(T4, 'FallLevel7.txt');